<x-user-layout>
    ...
    <section class="relative bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 py-16 md:py-24 overflow-hidden">
        ...
        <div class="container max-w-screen-xl mx-auto px-4 relative z-10">
            <div class="max-w-4xl mx-auto text-center">
                ...
                <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-6">{{ $kb->title }}</h1>
                <p class="text-xl text-blue-100 mb-6">
                    {{ $kb->excerpt ?? Str::limit(strip_tags($kb->content), 100) }}
                </p>
                <div class="flex flex-col sm:flex-row justify-center items-center gap-4 text-blue-100">
                    <div class="flex items-center">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($kb->user->name) }}&background=random"
                            alt="Author" class="w-10 h-10 rounded-full mr-3 border-2 border-white">
                        <span>{{ $kb->user->name }}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="far fa-clock mr-2"></i>
                        <span>Published: {{ $kb->published_at->format('F j, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-16">
        <div class="container max-w-screen-xl mx-auto px-4">
            ...
            <article class="max-w-4xl mx-auto" aria-labelledby="article-title">
                ...
                @if($kb->thumbnail)
                    <figure class="mb-10 rounded-2xl overflow-hidden shadow-lg transform hover:scale-[1.01] transition-transform duration-300">
                        ...
                        @if(!empty($kb->image_caption))
                            <figcaption class="text-sm text-gray-500 px-4 py-2 bg-white/70 backdrop-blur">
                                {!! $kb->image_caption !!}
                            </figcaption>
                        @endif
                    </figure>
                ...
                <!-- Social Sharing -->
                <div class="flex flex-col sm:flex-row justify-between items-center mb-10 p-6 bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div class="flex items-center space-x-3">
                        @php
                            ...
                            $title = urlencode($kb->title);
                            $thumb  = $kb->thumbnail ? urlencode(asset($kb->thumbnail)) : '';
                        @endphp
                        ...
                        <!-- WhatsApp -->
                        <a
                            href="https://api.whatsapp.com/send?text={{ urlencode($kb->title . ' ' . url()->current()) }}"
                            target="_blank" rel="noopener noreferrer"
                        ...
                        <!-- Email -->
                        <a
                            href="mailto:?subject={{ rawurlencode($kb->title) }}&body={{ rawurlencode(url()->current()) }}"
                            target="_blank" rel="noopener noreferrer"
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mt-10 p-6 bg-gray-50 rounded-2xl">
                    <div class="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                        ...
                        <span class="px-3 py-1 rounded-full bg-white text-gray-700 border">{{ $kb->category->name }}</span>
                        @if(!empty($kb->tags))
                            @foreach(explode(',', $kb->tags) as $tag)
                    ...
                    <div class="text-sm text-gray-500 flex items-center">
                        <i class="far fa-clock mr-2"></i>
                        @php
                            $words = str_word_count(strip_tags($kb->content));
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            <div class="flex space-x-2">
                                <button id="helpfulYes" type="button"
                                    ...
                                    aria-label="Yes, this article was helpful" title="Yes">
                                ...
                                <button id="helpfulNo" type="button"
                                    ...
                                    aria-label="No, this article was not helpful" title="No">
            ...
            @if($relatedKbs->count() > 0)
                <section class="mt-16">
                    ...
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($relatedKbs as $related)
                            <a href="{{ route('single-kb', $related->slug) }}"
            ...
            <!-- Support Section -->
            <section class="mt-16 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-8 border border-blue-200">
                <div class="text-center max-w-2xl mx-auto">
                    <div class="w-16 h-16 bg-blue-600 rounded-full flex items-center justify-center mx-auto mb-4">
                        <i class="fas fa-headset text-white text-2xl"></i>
                    </div>
                    <h2 class="text-2xl font-bold text-blue-800 mb-2">Need More Help?</h2>
                    <p class="text-blue-600 mb-6">Our support team is available 24/7 to assist you with any questions</p>
                    <a href="{{ route('contact') }}"
                       class="inline-block bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-medium transition-colors shadow-md">
                        Contact Support
                    </a>
                </div>
            </section>
    ...
    <div id="thanksModal" class="fixed inset-0 z-50 hidden items-center justify-center px-4 backdrop-blur-sm">
        <div class="absolute inset-0 bg-black/40" id="modalBackdrop"></div>
            ...
            <div class="mt-6 text-right">
                <button id="closeThanks" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-medium">
                    Close
                </button>
            </div>
        </div>
